...
const stopGame = (status) => {
  ...
  document.getElementById(
    "game"
  ).innerHTML += `<p>The word was: <span class = "result-word">${word}<span/><p/><button id = "play-again" class = "button-primary px-5 py-2 mt-5">Сыграть еще раз</button>`;
...
export const startGame = () => {
  ...
  gameDiv.innerHTML += `<p id="tries" class="mt-2">TRIES LEFT: <span id="tries-left" class="font-medium
 text-red-600">10<span/></p>`;
  ...
  document.getElementById("quit").onclick = () => {
    const isSure = confirm("Are you sure are want to quit and lose progress?");
    if (isSure) {
      stopGame("quit");
    }
  };
